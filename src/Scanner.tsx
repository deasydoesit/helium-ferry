  ...
  const handleScan = (data: string | null): void => {
    if (data) {
      ...
      try {
        ...
        if (obj.type === 'payment') {
          str = `helium-wallet -f /Volumes/RED/wallet.key.1 -f /Volumes/BLACK/wallet.key.2 --format json pay --payee ${
            obj.address
        ...
        if (obj.type === 'stake') {
          str = `helium-wallet -f /Volumes/RED/wallet.key.1 -f /Volumes/BLACK/wallet.key.2 --format json validators stake ${obj.validator} ${obj.stakeAmount} --fee ${obj.fee}`;
        }
        if (obj.type === 'unstake') {
